# Global Dev Team

## Project Context

- **Owner:** David Driscoll
- **Purpose:** Global day-to-day development acceleration — works across all projects
- **Stack:** .NET 10/11, C#, MSBuild, NuGet, Aspire, Blazor, PowerShell, Azure DevOps, Git/GitHub
- **Universe:** The Expanse

## Members

| Name | Role | Charter | Emoji |
|------|------|---------|-------|
| Holden | Lead / Architect | .squad/agents/holden/charter.md | 🏗️ |
| Naomi | Modern .NET Developer | .squad/agents/naomi/charter.md | ⚡ |
| Miller | Preview .NET Developer | .squad/agents/miller/charter.md | 🔬 |
| Amos | MSBuild / NuGet Developer | .squad/agents/amos/charter.md | 🔧 |
| Alex | PowerShell Specialist | .squad/agents/alex/charter.md | 💻 |
| Bobbie | Git / GitHub Specialist | .squad/agents/bobbie/charter.md | 🗃️ |
| Drummer | Azure DevOps Specialist | .squad/agents/drummer/charter.md | ⚙️ |
| Scribe | Session Logger | .squad/agents/scribe/charter.md | 📋 |
| Ralph | Work Monitor | .squad/agents/ralph/charter.md | 🔄 |
| Rai | RAI Reviewer | .squad/agents/rai/charter.md | 🛡️ |

## Issue Source

Not configured. Connect with: `"pull issues from {owner/repo}"`
